...
<div x-data="lotDetail()" x-init="init()">
    ...
    <div class="bg-slate-800/50 backdrop-blur border border-slate-700/50 rounded-xl mb-6 overflow-hidden">
        ...
        <div x-data="{ showDetails: false }" class="border-t border-slate-600">
            ...
            <div x-show="showDetails" x-collapse>
                <table class="w-full text-xs">
                    ...
                    <tbody class="text-slate-200">
                        @php $rowNum = 0; @endphp
                        @if($scheduleDisplayData && isset($scheduleDisplayData['schedules']))
                            @foreach($scheduleDisplayData['schedules'] as $scheduleData)
                            @php
                                $rowNum++;

                                // ALL data comes from service - NO calculations in Blade
        ...
        @if($allSchedulesData && isset($allSchedulesData['schedules']) && count($allSchedulesData['schedules']) > 12)
        <div x-data="{ showAllDetails: false }" class="border-t border-slate-600">
            <button @click="showAllDetails = !showAllDetails" class="w-full px-4 py-2 text-left text-xs text-slate-400 hover:bg-slate-700/30 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    @php
                        $totalSchedules = isset($allSchedulesData['schedules']) ? count($allSchedulesData['schedules']) : 0;
                    @endphp
                    <span class="text-[10px] px-2 py-0.5 bg-slate-700/50 text-slate-400 rounded">{{ $totalSchedules }} oy</span>
                </div>
            ...
            <div x-show="showAllDetails" x-collapse>
                <table class="w-full text-xs">
                    ...
                    <tbody class="text-slate-200">
                        @php $rowNum = 0; @endphp
                        @if($allSchedulesData && isset($allSchedulesData['schedules']))
                            @foreach($allSchedulesData['schedules'] as $scheduleData)
                            @php
                                $rowNum++;
                                // ALL data comes from service - NO calculations in Blade
                                $isOverdue = $scheduleData['is_overdue'];
