...
<div x-data="contractDetail()" x-init="init()">
    ...
    <div class="bg-white border rounded-lg mb-6">
        ...
        @php
            ...
            $examplePenya = 0;
            if ($exampleSchedule) {
                $exampleKun = \Carbon\Carbon::parse($exampleSchedule->oxirgi_muddat)->diffInDays(now());
                $examplePenya = $exampleSchedule->qoldiq_summa * 0.004 * $exampleKun;
                // Max 50% limit
                $maxPenya = $exampleSchedule->qoldiq_summa * 0.5;
                if ($examplePenya > $maxPenya) $examplePenya = $maxPenya;
            }
        @endphp
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @php
                        ...
                        if ($schedule->qoldiq_summa > 0 && \Carbon\Carbon::parse($schedule->oxirgi_muddat)->isPast()) {
                            $kechikish = \Carbon\Carbon::parse($schedule->oxirgi_muddat)->diffInDays(now());
                    ...
                    <tr class="hover:bg-gray-50 group" x-data="{ editing: false }">
                        ...
                        <td class="px-4 py-3 text-center">
                            ...
                            <div class="group/penya relative">
                                <span class="text-red-600 font-medium cursor-help">{{ number_format($schedule->penya_summasi ?? 0, 0, '', ' ') }}</span>
                ...
                <tfoot class="bg-gray-50 font-medium">
                    <tr>
                        ...
                        <td class="px-4 py-3 text-center text-red-600">{{ number_format($contract->paymentSchedules->sum('penya_summasi') - $contract->paymentSchedules->sum('tolangan_penya'), 0, '', ' ') }}</td>
                        <td colspan="2"></td>
                    </tr>
                </tfoot>
                @endif
        ...
        <div class="divide-y divide-gray-100">
            ...
            <div class="px-4 py-3 flex justify-between items-center">
                <div>
                    ...
                    <p class="text-xs text-gray-400">{{ $payment->tolov_turi ?? 'Naqd' }}</p>
                </div>
